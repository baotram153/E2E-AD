    ...
    for epoch in range(nepochs):
        for batchi, (imgs, rots, trans, intrins, post_rots, post_trans, pts, binimgs) in enumerate(loader):
            ...
            for si in range(imgs.shape[0]):
                ...
                imname = f'imgs/check{epoch:03}_{batchi:05}_{si:02}.jpg'
                print('saving', imname)
                plt.savefig(imname)
...
def viz_model_preds_carla(version,
                    modelf,
                    data_folder='/workspace/datasets/CARLA-data/data_collect_towndebug',
                    ...
                    bsz=4,
                    nworkers=10,    # should adjust this if the model uses too much GPU
                    ):
    grid_conf = {
        'xbound': xbound,
        'ybound': ybound,
    ...
    trainloader, valloader = compile_data(version, data_folder, data_aug_conf=data_aug_conf,
                                          grid_conf=grid_conf, bsz=bsz, nworkers=nworkers,
                                          parser_name='segmentationdatacarla')
    ...
    with torch.no_grad():
        for batchi, (imgs, rots, trans, intrins, post_rots, post_trans, binimgs) in enumerate(loader):
            out = model(imgs.to(device),       # should fix this to img
                    rots.to(device),
                    trans.to(device),
                    intrins.to(device),
                    post_rots.to(device),
            ...
            for si in range(imgs.shape[0]):
                ...
                plt.xlim((0, out.shape[3]))
                plt.ylim((out.shape[3]), 0)
                add_ego(bx, dx)

                imname = f'imgs/eval{batchi:06}_{si:03}.jpg'
                print('saving', imname)
                plt.savefig(imname)
                counter += 1
