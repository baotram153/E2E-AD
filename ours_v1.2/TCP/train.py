...
class TCP_planner(pl.LightningModule):
	...
	def validation_step(self, batch, batch_idx):
		...
		batch_throttle_l1 = 0
		for i in range(B):
			throttle, brake, steer = self.model.get_action(pred['mu_branches'][i], pred['sigma_branches'][i])
...
if __name__ == "__main__":
	...
	# TCP_model = TCP_planner.load_from_checkpoint("/workspace/log/TCP/chkpt/ours_v1/ours_v1.2/debug/best_epoch=02-val_loss=0.874.ckpt", config=config, lr=args.lr)
