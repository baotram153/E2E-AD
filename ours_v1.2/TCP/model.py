...
class TCP(nn.Module):
	...
	def __init__(self, config):
		...
		featuremap_decoder_layer = nn.TransformerDecoderLayer(d_model=256, nhead=8, batch_first=True)
		waypoint_decoder_layer = nn.TransformerDecoderLayer(d_model=256, nhead=8, batch_first=True)

		self.featuremap_transformer_decoder = nn.TransformerDecoder(featuremap_decoder_layer, num_layers=6)

		self.waypoint_transformer_decoder = nn.TransformerDecoder(waypoint_decoder_layer, num_layers=6)
	...
	def forward(self, img, state, target_point):
		...
		traj_hidden_state = torch.stack(traj_hidden_state, dim=1)	# B, 4, 256
		...
		traj_hidden_state_embedded = self.pos_embedding(traj_hidden_state)	# B, 4, 256
		# tgt = torch.cat([traj_hidden_state_embedded, measurement_feature.unsqueeze(dim=1)], dim=1)	# (batch_size, seq, emb)
		tgt = measurement_feature.unsqueeze(dim=1)	# ego query - shape: B, seq_len, d_model
		
		_ , n_channels, height, width = cnn_feature.shape
		memory =  cnn_feature.view(-1, n_channels, height*width)	# key, value: 512x256
		memory_256 = self.increase_dim(memory)

		featuremap_decoder_out = self.featuremap_transformer_decoder(tgt, memory_256)		# output dim: tgt - 1 x 256

		# out = torch.flatten(out, 1)	# (256*5, 1)
		waypoint_decoder_out = self.waypoint_transformer_decoder(tgt, traj_hidden_state_embedded)
		out = torch.cat([featuremap_decoder_out, waypoint_decoder_out], dim=2).squeeze(dim=1)
