...
class PositionalEncoding (nn.Module):
    ...
    def __init__(self, d_model, dropout=0.1, max_len=2048) -> None:
        ...
        self.pos_emb = self.pos_emb.unsqueeze(dim=0)

    def forward(self, X: Tensor):
        '''X.shape = [batch_size, seq_length, d_model]'''   # B, 4, 256
        with torch.no_grad():
            # print(X.shape, self.pos_emb[:, :X.shape[1], :].shape)
            X = X + self.pos_emb[:, :X.shape[1], :].to(X.device)     # d_model has already been specified
