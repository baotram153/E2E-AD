...
class CARLA_Data(Dataset):
	...
	def __init__(self, root, data_folders, img_aug = False):
		...
		for sub_root in data_folders:	# # combine data in multiple routes00
			...
			self.speed_error += data['speed_error']
			self.angle_error += data['angle_error']
			# print(len(self.angle_error))
			# print(len(self.action_mu))
			# exit()
	...
	def __getitem__(self, index):
		...
		data['speed_error'] = self.speed_error[index]
		data['angle_error'] = self.angle_error[index]
		# print(len(self.angle_error[index]), len(self.angle_error))
