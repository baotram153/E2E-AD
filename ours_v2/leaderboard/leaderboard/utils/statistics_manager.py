...
class RouteRecord():
    def __init__(self):
        self.route_id = None    # what's the difference between route's id and index?
        self.index = None
        self.status = 'Started'
        self.infractions = {
...
def to_route_record(record_dict):
    '''Convert a dictionary to a route record'''
    record = RouteRecord()
...
def compute_route_length(config):
    '''Calculate route length by summing up distances between pairs of waypoints'''
...
class StatisticsManager(object):
    ...
    def __init__(self):
        ...
        self._registry_route_records = []   # save records of routes traversed

    def resume(self, endpoint):
        ''' 
        - load the records from checkpoint to '_registry_route_records'
        - save '_registry_route_records' back to checkpoint
        '''
