...
class RouteIndexer():
    ...
    def resume(self, endpoint):
        '''
        - endpoint: file ..._result.json (called file result)
        - load the [_checkpoint][progress] (2 components: current route and total route) part of the dictionary from file result
            + if current route > total route -> announce
        '''
