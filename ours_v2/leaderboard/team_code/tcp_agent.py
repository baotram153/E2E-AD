...
class TCPAgent(autonomous_agent.AutonomousAgent):
	def setup(self, path_to_conf_file):
		...
		self.turn_controller = self.net.turn_controller
		self.speed_controller = self.net.speed_controller
	...
	def run_step(self, input_data, timestamp):
		...
		# print(gt_velocity, gt_velocity.shape)
		# exit()
		...
		pred= self.net(rgb, state, target_point, self.speed_controller._window, self.turn_controller._window)

		# throttle_traj, brake_traj, steer_traj, metadata_traj = self.net.control_pid(pred['pred_wp'], gt_velocity[0].data, target=target_point)	# actually doesn't use
		# print(pred_action.shape)
		# exit()
		throttle_traj, brake_traj, steer_traj, metadata_traj = self.net.process_single_action(pred["pred_action"][0])
		throttle_ctrl, brake_ctrl, steer_ctrl = self.net.get_action(pred['mu_branches'], pred['sigma_branches'])	# pred['action'].shape = (1,2)
		throttle_ctrl, brake_ctrl, steer_ctrl = throttle_ctrl[0].cpu().numpy(), brake_ctrl[0].cpu().numpy(), steer_ctrl[0].cpu().numpy()
		...
		# print(steer_traj, throttle_traj, brake_traj)
		# sleep(1)
		# control.steer = np.clip(self.alpha*steer_ctrl + (1-self.alpha)*steer_traj, -1, 1)
		# control.throttle = np.clip(self.alpha*throttle_ctrl + (1-self.alpha)*throttle_traj, 0, 0.75)
		# control.brake = np.clip(self.alpha*brake_ctrl + (1-self.alpha)*brake_traj, 0, 1)
